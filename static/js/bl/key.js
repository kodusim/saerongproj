/* 작가 키 — 계정이 없는 1차에서 "이 작품은 내 것"의 유일한 근거.

   테스트 기간에는 서버(app/routers/bl.py 의 OPEN_TEST)가 이 헤더를 무시하고
   모두를 같은 작가로 취급한다. 그래서 ensureKey/setKey 는 지금 쓰이지 않지만,
   테스트가 끝나면 다시 필요하므로 남겨 둔다.

   IP 로 가르면 공유기 재접속·모바일 전환만으로 작가가 수정 권한을 잃는다.
   그래서 브라우저가 만든 랜덤 토큰을 localStorage 에 두고 헤더로 보낸다.
   2차에서 계정이 붙으면 이 키를 그대로 user 에 승계한다. */
