...
![](/assets/tossauth_flow.BzgVhGds.png)

## ① AccessToken 받기

토스 본인확인을 위한 **Access Token**을 발급받아요.\
발급된 토큰은 이후 호출하는 모든 서비스 API의 **인증(Authorization)** 용도로 사용돼요.

토큰에는 **만료 시간(`expires_in`)** 이 있어요. 만료되면 새 토큰을 발급받아야 해요.\
이미 유효한 토큰이 있으면 중복 발급을 피해서 불필요한 API 호출을 줄여주세요.
...
* `USER_NONE`\
  개인정보를 보내지 않고 토스앱을 바로 호출해 한 번에 인증을 끝내는 **원터치 방식**이에요.\
  → 본인확인 요청에서 `requestType`을 `USER_NONE`으로 설정하세요.
* `USER_PERSONAL`\
  **이름·생년월일·전화번호** 를 기반으로 인증을 요청해요.\
  미니앱에서 위 정보를 입력받아 서버로 전달해 주세요.\
  → `userName`·`userPhone`·`userBirthday`·`sessionKey`가 필요해요.
...
| requestType | string | Y | `USER_NONE` : 토스 표준창을 호출하는 방식으로 토스 웹 페이지에서 고객 정보를 입력받아 인증 요청  (표준창 사용에는 triggerType 선언 불필요) `USER_PERSONAL` : 고객의 이름, 생년월일, 전화번호 정보를 기반으로 인증 요청 (암호화된 `userName`·`userPhone`·`userBirthday` 정보 필수) |
| triggerType | string | N | `USER_PERSONAL`일 때만 전달: `APP_SCHEME` |
| userName | string | N | `USER_PERSONAL`일 때 필수, [암호화](/bedrock/reference/framework/인증/tosscertEncrypt.html) 필수 |
| userPhone | string | N | `USER_PERSONAL`일 때 필수, 숫자만, [암호화](/bedrock/reference/framework/인증/tosscertEncrypt.html) 필수 |
| userBirthday | string | N | `USER_PERSONAL`일 때 필수, `YYYYMMDD`, [암호화](/bedrock/reference/framework/인증/tosscertEncrypt.html) 필수 |
| sessionKey | string | N | `USER_PERSONAL`일 때 필수, AES 암복호화용, 매 요청마다 신규 생성 |

**`requestType`이 `USER_PERSONAL`일 경우**
...
| success.androidAppUri | string | 안드로이드 인증 앱 스킴 값으로 appScheme과 같은 역할을 하지만, Chrome Intent를 사용하기 때문에 고객사의 추가 기능 구현없이 토스 앱 설치 유무를 판별할 수 있는 장점이 있습니다. (`triggerType=APP_SCHEME` 일 경우만) |
| success.iosAppUri | string |  iOS 인증 앱 스킴 값으로 appScheme과 같은 역할을 하지만, Universal Link를 사용하기 때문에 안드로이드와 마찬가지로 고객사의 추가 기능 구현 없이 토스 앱 설치 유무를 판별할 수 있는 장점이 있습니다.(`triggerType=APP_SCHEME` 일 경우만) |
