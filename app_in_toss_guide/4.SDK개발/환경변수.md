...
`plugin-env`는 프로젝트에 환경 변수를 주입해\
`import.meta.env`로 쉽게 접근할 수 있도록 도와주는 플러그인이에요.

이 플러그인은 **React Native 환경에서만 사용 가능**하며,\
`@granite-js/react-native` 기반의 설정 파일(`granite.config.ts`)에서 적용돼요.

## 설치

패키지 매니저에 따라 아래 명령어 중 하나로 설치할 수 있어요.
