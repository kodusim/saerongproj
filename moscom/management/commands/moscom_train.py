...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        # 시간순으로 나눠야 정확한 평가지만 데이터 적으니 random split
        ...
        self.stdout.write(self.style.NOTICE('4) 평가 지표'))
        for k in ('train_mae', 'train_rmse', 'train_r2', 'test_mae', 'test_rmse', 'test_r2'):
            self.stdout.write(f'   {k:>10s}: {report[k]:.3f}')
        self.stdout.write(f'   target_mean: {report["target_mean"]:.2f}, max: {report["target_max"]}, min: {report["target_min"]}')
