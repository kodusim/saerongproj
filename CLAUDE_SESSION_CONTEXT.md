...
- **표 열 너비는 `.gw-table-wrap` 의 CSS 변수(`--w-*`) 로 고정돼 있다.** 예전엔
  드래그로 조절하고(`columns.js`) 문서함·자료실·공지사항 사이에 동기화까지 했는데,
  표 3개가 서로 다른 DOM/저장 키를 써서 계속 "따로 논다"는 불만이 나왔다. 2026-08-26
  에 리사이즈 기능 자체를 걷어내고 CSS 기본값으로 고정했다 — `columns.js` 는 삭제.
